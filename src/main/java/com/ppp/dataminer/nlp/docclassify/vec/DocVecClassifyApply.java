 ...
 *
 */
public class DocVecClassifyApply {
	    // weka数据格式
		...
		static {
			File resumeStructFile = new File("model/docVec.arff");
			...
			try {
				...
				classify = (Classifier) SerializationHelper.read("model/docvecsmo.model");
		...
		public static List<WordPair> docClassify(String position, String content) {
			float[] vec = genDocVec(position, content);
			return docClassify(vec);
		}

		private static List<WordPair> docClassify(float[] vec) {
			List<WordPair> classifyList = new ArrayList<WordPair>();
			Instance inst = new DenseInstance(101);
			inst.setDataset(instances);
			// 把数据放入instance对象中
			inst.setValue(100, "计算机软件");
			for (int i = 0; i < 100; i++) {
				inst.setValue(i, vec[i]);
			}
			instances.setClassIndex(100);
			try {
				// 各分类的分值
