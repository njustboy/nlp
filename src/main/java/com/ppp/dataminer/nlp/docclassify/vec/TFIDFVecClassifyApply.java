...
/**
 * 使用tfidf支持向量作为特征进行分类建模
 * @author zhangwei
 *
 */
public class TFIDFVecClassifyApply {
	// weka数据格式
	...
	static {
		File resumeStructFile = new File("model/tfidfVec.arff");
		...
		try {
			...
			classify = (Classifier) SerializationHelper.read("model/keywordtfidfvecsmo.model");
	...
	public static List<WordPair> docClassify(String position, String content) {
		double[] vec = genTFIDFVec(position, content);
	...
	private static List<WordPair> docClassify(double[] vec) {
		...
		Instance inst = new DenseInstance(61);
		inst.setDataset(instances);
		// 把数据放入instance对象中
		inst.setValue(60, "计算机软件");
		for (int i = 0; i < 60; i++) {
			inst.setValue(i, vec[i]);
		}
		instances.setClassIndex(60);
		try {
			// 各分类的分值
	...
	public static double[] genTFIDFVec(String title, String content) {
